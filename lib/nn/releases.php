...
/*
	File obtained from:
		https://github.com/Aaron-/nnplus/blob/momo/www/lib/releases.php
...
{	
	...
	function processReleases() 
	{
		...
		//
		$max_id = $db->queryOneRow("SELECT MAX(ID) as maxid FROM parts");
		$max_id_dateadded = $db->queryOneRow(sprintf("select max(ID) as maxid2 from parts where dateadded < %s - interval %d day", $db->escapeString($currTime["now"]), $page->site->rawretentiondays));
		
		$max_id_binary = $db->queryOneRow("SELECT MAX(ID) as maxid FROM binaries");
		$max_id_binary_dateadded = $db->queryOneRow(sprintf("select max(ID) as maxid2 from binaries where dateadded < %s - interval %d day", $db->escapeString($currTime["now"]), $page->site->rawretentiondays));
		
		echo "Deleting parts which are older than ".$page->site->rawretentiondays." days\n";			
		
		// When there are no other parts/binaries truncate is much faster compared to delete from.
		if($max_id['maxid'] == $max_id_dateadded['maxid2'])
		{
		...
		if($max_id_binary['maxid'] == $max_id_binary_dateadded['maxid2'])
		{
			echo "Truncating table\n";
